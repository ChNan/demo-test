...
function formatRange(date1, date2, formatStr, separator, isRTL, noChunks) {
	var localeData;
	...
	return formatRangeWithChunks(
		date1,
		date2,
		noChunks ? formatStr : getFormatStringChunks(formatStr),
		separator,
		isRTL,
		noChunks
	);
...
function formatRangeWithChunks(date1, date2, chunks, separator, isRTL, noChunks) {
	...
	if (noChunks) {
		leftStr = formatDate(date1, chunks);
		middleStr = separator;
		rightStr = formatDate(date2, chunks);
	}
	else {
...
Grid.mixin({
    ...
    handleSegDblclick: function(seg, ev) {
        var res = this.view.trigger('eventDblclick', seg.el[0], seg.event, ev); // can return `false` to cancel
        if (res === false) {
            ev.preventDefault();
        }
    },
...
var DayTableMixin = FC.DayTableMixin = {
	...
	renderHeadTrHtml: function() {
		return '' +
			'<tr>' +
				...
				this.renderHeadDateCellsHtml(1) +
				(this.isRTL ? this.renderHeadIntroHtml() : '') +
			'</tr>'
			+'<tr>'+
			this.renderHeadDateCellsHtml(2) +
			(this.isRTL ? this.renderHeadIntroHtml() : '') +
			'</tr>'
			;
	},
	...
	renderHeadDateCellsHtml: function(type) {
		var htmls = [];
		var col, date;

		for (col = 0; col < this.colCnt; col++) {
			date = this.getCellDate(0, col);
			htmls.push(this.renderHeadDateCellHtml(type,date));
		}
		return htmls.join('');
	},
	...
	renderHeadDateCellHtml: function(type, date, colspan, otherAttrs) {
		var view = this.view;
		var today = view.calendar.getNow();
		var res = '' +
			'<th style="height: 30px;" class="fc-day-header ' + view.widgetHeaderClass + ' fc-' + dayIDs[date.day()]
			+ (type == 1?(date.isSame(today, 'day') ? ' fc-day-header-today':' fc-day-header-other'):'')
			+ '"' +
			(this.rowCnt === 1 ?
			...
			view.buildGotoAnchorHtml(
				{ date: date, forceOff: this.rowCnt > 1 || this.colCnt === 1 ,fctp:type},
...
var View = FC.View = Class.extend(EmitterMixin, ListenerMixin, {
	...
	formatRange: function(range, formatStr, separator) {
		...
		return formatRange(range.start, end, formatStr, separator, this.opt('isRTL'), this.opt('noChunks'));
	},
...
var agendaTimeGridMethods = {
	...
	renderHeadIntroHtml: function() {
		...
		if (view.opt('weekNumbers')) {
			...
			return '<th rowspan="2" class="fc-axis ' + view.widgetHeaderClass + '" ' + view.axisStyleAttr() + '></th>';
		}
	},
