 ...
 * });
    ...
    var openTag = '{[';
    var closeTag = ']}';
    $.i18n.htmlFmter = function (htmlText) {
        //加一个标签，用来跳过不格式化的页面
        if(!htmlText || htmlText.substring(0,12) =='<!--false-->'){
            return htmlText;
        }
        var tmp,
            result = "",
            i = 0;
        var res = htmlText.split(openTag);
        var len = res.length;

        for(;i<len;i++){
            tmp = res[i].split(closeTag);
            if(tmp.length === 1){
                result += tmp[0];
            }else{
                result += getKeyValue(tmp[0]) + tmp[1];
            }
        }
...
// TODO key parts that start with numbers quietly fail. i.e. month.short.1=Jan
    ...
    function getKeyValue(key){
        return key in $.i18n.map?$.i18n.map[key]:'['+key+']';
    }
