  ...
  * ================================= */
  ...
  , highlighter: function (item) {
      ...
      return item.replace(new RegExp('(' + query + ')', 'ig'), function ($1, match) {
        return '<strong style="color:#12B5B0;">' + match + '</strong>'
      })
    }
  ...
  , next: function (event) {
      var a = this.$menu.find('.active')[0];
      var top = $(a).position().top;
      var scroll_height = this.$menu.find('li').height();
      if(top + scroll_height > this.$menu.height()) {
        $(this.$menu).scrollTop($(this.$menu).scrollTop() + scroll_height);
      }
  ...
  , prev: function (event) {
      ...
      if (!prev.length) {
        ...
        $(this.$menu).scrollTop(this.options.items * scroll_height);
      }
  ...
  , focus: function (e) {
      ...
      $('body').off( "mousedown").on( "mousedown", function( event ) {
          ...
          var _this = this;
          if($(targetObj).closest(".typeahead.dropdown-menu").length == 1) {
              event.preventDefault();
              _this.cancelBlur = true;
          } else {
              _this.cancelBlur = false;
          }
      });
    }
  ...
  , blur: function (event) {
      this.focused = false
      var targetObj = event.target;
      if (!this.mousedover && this.shown && !this.cancelBlur) {
        $('body').off( "mousedown");
        this.hide();
      }
    }
